...
(
    ...
    WHERE COALESCE(REP_RES_DATE.x.value('./@m', 'nvarchar(5)'), '1') = COALESCE(REP_OUTST_PR.x.value('./@m', 'nvarchar(5)'), '1')
AND x.RECID = a.RECID
) AS REP_OUTST_PR,
(
    ...
    WHERE COALESCE(REP_RES_DATE.x.value('./@m', 'nvarchar(5)'), '1') = COALESCE(REP_OVERDUE_DAYS.x.value('./@m', 'nvarchar(5)'), '1')
AND x.RECID = a.RECID
) AS REP_OVERDUE_DAYS,
(
    ...
    WHERE COALESCE(REP_RES_DATE.x.value('./@m', 'nvarchar(5)'), '1') = COALESCE(REP_RES_SUBS.x.value('./@m', 'nvarchar(5)'), '1')
AND x.RECID = a.RECID
) AS REP_RES_SUBS
