SELECT TOP 100 LEFT(RECID, CHARINDEX(';', RECID)-1) AS REF_NO,
       ...
       XMLRECORD.value('(/row/c5/text())[1]', 'char(3)') AS DEBIT_CURRENCY,
       XMLRECORD.value('(/row/c6/text())[1]', 'decimal(19,2)') AS DEBIT_AMOUNT,
       ...
       XMLRECORD.value('(/row/c13/text())[1]', 'char(3)') AS CREDIT_CURRENCY,
       XMLRECORD.value('(/row/c14/text())[1]', 'decimal(19,2)') AS CREDIT_AMOUNT,
	   ...
	   XMLRECORD.value('(/row/c177/text())[1]', 'smallint') AS CURR_NO,
	   XMLRECORD.value('(/row/c178/text())[1]', 'nvarchar(40)') AS INPUTTER,
	   CAST(SUBSTRING(XMLRECORD.value('(/row/c179/text())[1]', 'nvarchar(35)'), 1, 6) AS DATE) AS LAST_UPD_DATE,
       XMLRECORD.value('(/row/c181/text())[1]', 'nvarchar(11)') AS CO_CODE,
       XMLRECORD.value('(/row/c182/text())[1]', 'nvarchar(4)') AS DEPT_CODE
