SELECT XMLRECORD.value('(/row/@id)[1]', 'nvarchar(35)') AS CONTRACT_NUMBER,
       TOT_OVRDUE_TYPE.x.value('(./text())[1]', 'nvarchar(35)') AS TOT_OVRDUE_TYPE,
...
WHERE COALESCE(TOT_OVRDUE_TYPE.x.value('./@m', 'nvarchar(5)'), '1') = COALESCE(TOT_OD_TYPE_AMT.x.value('./@m', 'nvarchar(5)'), '1');